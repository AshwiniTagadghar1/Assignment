...
const Students = ()=> {
  ...
  const id = location?.search.split('id=')[1]|| null;
  console.log(id)
    const [origdata,setorigData]=React.useState([])
  const [data,setData]=React.useState([])
  React.useEffect(()=> {
    axios.get(`http://localhost:8049/api/student?id=${id}`).then(x=> {
     // const datax = x.data;
...
const handleClick = ()=> {
  const  y = origdata.filter(x => x.name.includes(studentFilter));
  setData(y);
}
const handleClear = ()=>   {
  setStudentFilter('');
  setData(origdata)};
    ...
    <TextField  label="Student Name" variant="standard" placeholder="Search student name" fullWidth value={studentFilter} onChange={(e)=>setStudentFilter(e.target.value) } /></div>
